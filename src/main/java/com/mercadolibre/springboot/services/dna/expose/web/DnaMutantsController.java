...
public class DnaMutantsController {
	...
	consumes = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Human> getStatistics() throws ApiException{
		//System.out.print(humanService.getQuantityHuman().stream().filter(p -> p.getKindHuman()==1));
		//List<Human> humans = humanService.getQuantityHuman().stream().filter(p -> p.getKindHuman()==1).collect(Collectors.toList());
			return new  ResponseEntity<Human>(HttpStatus.ACCEPTED);
